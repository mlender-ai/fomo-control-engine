...
### 시간축 규약 (WO-56 · 캔들 앵커)
**상태는 확정 캔들에서만 전진한다.** persist·EMA·candles_in_state는 호출 횟수가 아니라 확정 캔들 단위로만 움직인다 — persist=2는 정확히 "확정 캔들 2개 연속"이며, 워커 90s/스카우트 15m/온디맨드 연타 어떤 호출 주기에서도 관성이 동일하다.

- **앵커**: `stance_state.last_bar_at` = 마지막으로 상태를 전진시킨 **확정** 캔들 시각. 스냅샷의 마지막 캔들이 미마감이면 그 직전 캔들이 앵커(`_confirmed_bar_iso`: `now − last_candle_at < 1캔들`이면 `last_candle_at − 1캔들`).
- **동결**: 새 관측의 앵커가 `last_bar_at`과 같거나(같은 캔들) 과거이거나(역행 데이터) 미상(타임스탬프 결손)이면 상태 무변 반환 — EMA·persist·candles 전진 없음, `flipped=False`. 같은 확정 캔들 = 같은 상태(재현성; 비교 대상은 preview 제외한 상태 본체).
- **프리뷰**: 동결 중에도 최신 raw 관측은 `stance_state.preview{raw_stance, long_score, short_score, as_of}`로 노출 — **UI 잠정 표시 전용, 알림·원장 사용 금지**. 미마감 노이즈는 프리뷰에만 새겨진다.
- **갭**: 여러 캔들 갭(미스캔 구간) 후 전진 시 관측은 1스텝(EMA·persist 1회)이지만, 유지 상태의 `candles_in_state`에는 경과 확정 캔들 수를 반영한다(상태가 그 기간 유지됐다는 사실 기록). 관측이 1개뿐이므로 persist를 갭만큼 가속하지 않는다.
- **flip 정렬**: `flipped=True`는 확정 캔들 전진에서만 발생 → `stance_flipped` 알림·judgment 기록이 자동으로 캔들 마감 정렬.
- **마이그레이션**: `last_bar_at` 없는 구버전 상태는 다음 확정 관측에서 1회 전진하며 앵커를 획득.

검증: `tests/test_directional_time_axis.py` — 동일 캔들 N회 호출 상태 동일 · 90초×20회 무flip(3분 관성 붕괴 수리) · 미마감 스파이크 preview만 변동 · 확정 2캔들 역전 flip · 부트스트랩 무flip · 갭 규약 · 역행 동결.

### insufficient·bootstrap
- `raw_stance == insufficient`(증거 부족): 방향 게이트가 이긴다 — 단 **확정 캔들 전진에서** 적용된다. 같은 캔들 내 일시 데이터 결손(앵커 미상)은 held 상태를 지우지 못하고 동결 + 프리뷰만 갱신(WO-56).
- 직전 상태 없음(bootstrap): 관성 없이 `raw_stance` 즉시 채택. **`flipped`는 항상 False** — 재시작·미스캔 심볼에서 flip 오발화 방지(WO-56).
...
합성 80.8%를 실제 성적으로 오인하지 않도록 Bitget 4h 확정봉을 per-bar 재생하고 T+24h net 방향 결과를 비중첩 채점한다. 2026-07-20 최초 420봉 결과는 BTC 36.6%(CI 22.0~51.2, N=41), ETH 51.1%(35.6~64.4, N=45), SOXL 36.4%(21.2~51.5, N=33)였다. 세 데이터셋 품질은 100/100이었다. 이후 기본 창은 2,196봉(약 1년) 영속 캐시로 확대했고, v1/v2를 동일 표본으로 비교한다.

페이지 경계 연속성까지 검증한 기본 창 결과는 BTC v2 46.1%(CI 40.6~51.5, N=293), ETH v2 44.3%(38.5~50.0, N=296), SOXL v2 47.1%(35.3~60.3, N=68)다. 각 v1은 44.6%, 44.9%, 42.9%였고 세 비교 모두 CI가 겹쳐 개선을 주장하지 않는다. BTC·ETH는 2025-07-17부터, 공개 이력이 짧은 SOXL은 2026-04-09부터 관측했다.

이 결과는 방향 엔진 v2가 합성 시나리오에서 진동을 줄인 것과 실제 미래 방향을 맞히는 능력이 같은 주장이 아님을 확인한다. 실데이터 signature는 v1/v2별로 분리 저장하며 합성 결과와 합산하지 않는다. CI가 겹치면 v2 개선을 주장하지 않으며 성적을 파라미터 승격 근거로 자동 사용하지 않는다.
