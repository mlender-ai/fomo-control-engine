...
합성 80.8%를 실제 성적으로 오인하지 않도록 Bitget 4h 확정봉을 per-bar 재생하고 T+24h net 방향 결과를 비중첩 채점한다. 2026-07-20 최초 결과는 BTC 36.6%(CI 22.0~51.2, N=41), ETH 51.1%(35.6~64.4, N=45), SOXL 36.4%(21.2~51.5, N=33)였다. 세 데이터셋 품질은 100/100이었다.

이 결과는 방향 엔진 v2가 합성 시나리오에서 진동을 줄인 것과 실제 미래 방향을 맞히는 능력이 같은 주장이 아님을 확인한다. 실데이터 signature는 `directional_v2_real_history_24h`로 분리 저장하며 합성 결과와 합산하지 않는다. 성적 악화를 숨기거나 파라미터 승격 근거로 자동 사용하지 않는다.
