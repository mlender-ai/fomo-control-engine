...
### `STRUCTURALLY_BLOCKED` 처리 — 택일 규칙 (WO-FCE-SAMPLE-RATE-01 Phase 4)

판정이 확정됐다: 보유 9건 중 8건이 2027-01-01 만기 → **검증 창 내 정산 예정 0건**.
`poly-v3` 만기 상한이 들어갔으므로 **표본은 신규 진입분으로만 생긴다.**

기존 보유 8건은 **미청산 유지**한다. 소급 청산·강제 정리는 엔진 판단이 아닌 결과를 성과로
계산하게 만든다 — 그건 표본이 아니라 조작이다.

#### 실측 (호스트에서 1회)

```bash
cd backend && python3 scripts/sample_viability_report.py ~/fomo_control_engine.db
# PHASE 2 블록: 만기 구간별 분포 · 구조적 차단 여부 · 정산 지연 실측
curl -s localhost:8875/api/system/paper/diagnosis | python3 -m json.tool | grep -A 30 '"poly"'
```

산출: `poly-v3` 적용 후 신규 진입 건수 · 그 만기 분포 · 남은 기간 내 정산 가능 건수
(= 진입률 × 남은 일수 × 만기 도래율).
...
| 실측 결과 | 선택 |
|---|---|
| 남은 기간 정산 가능 건수 ≥ 목표 표본 | **그대로 진행** |
| 0 < 정산 가능 < 목표 | **유니버스 확대** 검토 (`SAMPLE_RATE.md` §3 용량 축만) |
| 정산 가능 ≈ 0 | **트랙 유보** — 이번 검증 대상에서 제외하고 사유 기록 |

**유보는 실패가 아니라 정직한 범위 축소다.** 표본이 안 나오는 트랙을 검증 대상에 남겨 두면
전체 검증 완료가 영원히 오지 않고, 그 사실이 다른 트랙의 결과까지 가린다.

만기 상한을 늘려 "정산 가능"을 만드는 것은 **금지**다 — 그건 검증 창 밖 만기를 다시
받아들이는 것이고, 이 필터가 존재하는 이유를 되돌리는 것이다.
