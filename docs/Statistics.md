...
> **필요 표본 재산정 진행 중** (WO-FCE-SAMPLE-RATE-01 Phase 3):
> 현재 우위 판정은 두 95% CI 의 **비겹침**을 요구하며, 이는 α=5% 검정이 아니라
> 실효 유의수준 약 **0.3%** 의 검정이다. 같은 검정력을 얻는 데 필요한 표본이
> 대안 방법의 2~4배다. 재산정 결과와 선택지: `docs/validation/REQUIRED_SAMPLE.md`.
> **채택 전까지 현재 방법을 유지한다.**
