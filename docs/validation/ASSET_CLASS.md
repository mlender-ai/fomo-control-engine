...
### 결정 (2026-08-23) — **1번 채택, 단 무데이터는 제3상태**

`EARNINGS-SUPPLY-01` 이 1번(공급원 배선)을 채택했다. 그리고 무데이터 처리는 원안의
"차단 유지"에서 **`not_evaluable`(판정 제외)** 로 정정됐다.

정정 근거는 이 문서가 몰랐던 사실이다 — **KR 주식 트랙이 이미 같은 문제를 풀어 뒀다**:

```python
stock_paper/policy.py:66      status="not_evaluable" · required=False
stock_paper/parameters.py:56  불변식으로 "공급원 연결까지" 를 강제
```

원안대로 차단하면 두 엔진이 같은 상황을 정반대로 처리하고 이 불변식과 충돌한다.
그리고 지금 차단해도 **얻는 것이 없다** — 262종은 crypto 라 게이트를 안 받고, 올바르게
stock 인 소수는 이미 `stage2_template` 에서 표본 0이다.

**순서가 안전장치다**:

```
공급원 배선 → required=True 전환 → 그다음 분류 수리(3-2)
```

분류 수리 시점에는 실적 게이트가 작동 중이므로 262종이 무방비로 거래되는 구간이 없다.
