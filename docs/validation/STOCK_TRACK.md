...
## 정지 트랙의 열린 포지션 방침 (WO-FCE-REPORT-DEFECTS-01 7-4)

`stock_us` 가 체결 invariant 로 정지한 상태에서 **미실현 +51.32 가 쌓이고 있었다.**
`stop_track` 은 신규 주문을 막지만 **이미 열린 포지션의 처리 방침이 없었다.**

### 방침 — (a) 신규 진입만 차단

| 안 | 판정 |
| --- | --- |
| (a) 출구 규칙대로 계속 평가·청산 | **채택** |
| (b) 정지 시점 전량 청산 | 기각 |
| (c) 동결 — 평가도 멈춤 | 기각 |

**(b) 를 기각한 이유가 결정적이다**: 정지 사유가 *체결가 산출*이다. 체결가를 못 믿어서
멈췄는데 그 상태로 전량 청산하면 **믿을 수 없는 가격으로 청산**하는 것이다. 정지가
막으려던 바로 그 위험을 대규모로 실행하게 된다.

**(c) 를 기각한 이유**: 평가를 멈추면 미실현이 정지 시점에 얼어붙는다. 그 값은 시간이
지날수록 현실과 멀어지고, 멀어진 값이 화면에 남으면 그것이 거짓이 된다.

### 단, 청산가도 같은 invariant 를 통과해야 한다

(a) 는 "정지를 무시하고 청산한다"가 아니다. 출구 판정은 계속하되 **체결은 정지를 푼 
invariant 를 통과한 가격에서만** 이루어져야 한다. 그 배선은 이 WO 범위 밖이다 — C1 이
출구 로직 수정을 금지한다.

### 표시

정지 기간 미실현은 **별도 표기**한다. 정상 트랙 미실현과 섞지 않는다:

```
  실현  +3.82 · 미실현 +51.32 · 정지 중 평가액(청산 안 됨)
```

그리고 D1 수리로 그 51.32 는 **현재 자본에서 빠졌다.** 이전에는 자본에 섞여 정지 트랙이
벌고 있는 것처럼 보였다.

**임의 청산 0건**(C6). 이 WO 는 원장을 쓰지 않는다.

---

## 체결 봉 불일치 수리 (WO-FCE-MAKE-IT-RUN-01 Phase 1 · 2026-08-31)

US 트랙 정지와 KR 큐 13,934건 보류의 **공통 원인**이었다. 별건이 아니라 논리 오류다.

### 결함 ① — 가격을 만든 봉과 검증하는 봉이 달랐다

```python
base = observation.session_open_price if order.reason == "session_closed" else observation.minute_close
...
if not observation.minute_low <= fill_price <= observation.minute_high:   # ← 현재 분봉
    raise FillInvariantViolation
```

시가는 **개장 직후 분봉에만** 있다. 체결이 그 분봉에서 일어나지 않으면 범위 밖이 되고,
갭이 있는 날이면 **반드시** 터진다.

**수리 (a 안 채택)**: `session_closed` 주문도 **체결 시점 분봉**(`minute_close`)으로
체결가를 만든다. 큐 주문이 개장 첫 분봉에 몰리는 것을 피하고, **가격 출처와 검증 봉이
같아진다.**

> **갭 정직성은 그대로다.** 금지사항이 지키려던 것은 "시가"가 아니라 **마법 체결 금지**다 —
> 신호가(무효화선)에 체결되면 손절 성적이 실제보다 좋아진다. 갭 이후 관측된 봉에서
> 파생되는 한 그 보호는 유지된다.

### 결함 ② — 스프레드가 범위를 밀어냈다 (WO 가 지목하지 않은 것)

```
minute_close == minute_high · 매수 · 반 스프레드 → raw = high + spread → 💥
```

분봉 종가가 고가와 같은 것은 흔하고(상승 마감), `round_to_tick` 은 매수에서 **올림**이라
한 틱 더 민다. **①만 고치면 이것이 계속 정지를 만든다.**

**수리**: 스프레드·틱 보정분을 실제 거래 범위 안으로 되돌린다(`_bounded_fill_price`).
그 분에 거래되지 않은 값으로는 체결될 수 없고, 범위 끝은 **그 분의 가장 불리한 실제
체결가**이므로 완화가 아니라 현실 안에서의 보수적 선택이다.

### invariant 는 완화하지 않았다 (C1)

검사는 그대로 있고 여전히 발화한다. **종가가 자기 봉 범위 밖이면 아무것도 자르지
않는다** — 그 관측은 모순이고 자르면 데이터 결함을 덮는다. 발화 자리가 "모형 문제"에서
"데이터 문제"로 좁혀졌을 뿐이다.

### 큐 적체를 근원에서 막았다 (항목 5)

마감 중에는 체결이 안 되므로 청산 신호가 매 틱 다시 발화하고, 그때마다 같은 심볼에 새
주문이 쌓였다. **13,934건은 소수 심볼의 중복이다.**

상한이 아니라 **중복 제거**다 — 같은 (시장·심볼·방향)에 대기 주문이 있으면 새로 만들지
않는다. 상한을 두면 상한만큼은 여전히 중복이 쌓인다.

### 재제출 배치 상한 (항목 4 · C4)

`FCE_STOCK_PAPER_REQUEUE_BATCH_LIMIT=200`. 개장 순간 큐 전체가 나가면 잡 실행 시간이 큐
길이에 비례하고, 그것이 `DISCOVERY-UNBLOCK-01` 의 장애 기전이다. **남은 건은 버리지
않는다** — `deferred` 로 조회되고 다음 실행이 이어받는다.

### 아직 하지 못한 것 — 호스트가 필요하다

| 항목 | 왜 |
| --- | --- |
| US 정지 해제 | 원장 상태 변경 — 수리 확인 후 호스트에서 |
| KR 큐 보류 해제 (`hold_queued=false`) | **US 확인 후**(C9) — 동시에 풀지 않는다 |
| 전략 표본 1건 이상 생성 | 시장이 열려 있어야 한다 |
| 잡 실행률 전후 | 개장 시각 실측 |
