...
## 생존 신호는 푸시하지 않는다 (2026-08-16 사용자 지시)
...
| 경로 | 이전 | 현재 |
| --- | --- | --- |
| 앱 내 `engine_liveness`·`process_restarted`·`data_stall`·`infra_capacity`·`job_backoff_stuck` | 발송(뮤트 관통) | **강등 — 미발송** |
| 외부 `deadman.sh` 사망·복구·자가점검 | 텔레그램 발송 | **로그 기록만** (`FCE_DEADMAN_PUSH=1` 로 복구 가능) |
| 일일 요약의 생존 라인 | 포함(뮤트 관통) | **제거** |
| 뮤트 중 일일 요약 | 생존 라인만 발송 | **아무것도 발송 안 함** |
| 포지션 위험 경보(`invalidation_breach` 등) | 발송 | **발송 유지** |

### 강등이지 삭제가 아니다

감시·판정·재시작·기록은 **전부 그대로 돈다.** 발송만 뺐다. 조회 경로:

```bash
curl -s localhost:8875/api/system/paper/diagnosis   # liveness · observation_integrity
tail -f logs/supervisor.log                         # 매달림 감지·재시작
ls logs/hang-dumps/                                 # 매달림 스택 덤프
```

이전 WO 들이 세운 "침묵이 스스로를 은폐한다" 원칙은 **발송이 아니라 조회로** 담보한다.

### 요약에 무엇이 들어가나

`app/notify/content_summary.py` — 진입·청산 / 폴리마켓 / 리더보드 세 블록.

- **0건도 적는다.** 줄이 없으면 "알림이 안 온 건가"와 구분되지 않는다.
- 폴리는 보유 건수가 아니라 **검증 창 내 정산 예정 건수**를 적는다. 0이면 "이 창에서는
  표본이 생기지 않습니다"를 매번 붙인다 — 조용히 두면 "아직 안 나왔나 보다"로 읽힌다.
- 리더보드는 **선정 기준(quality_score)과 사후 승률을 구분**해 적는다. 표본 부족이면 명시한다.
- 조회 실패는 그 줄만 "미산출 + 사유"가 되고 요약 전체를 막지 않는다.
