...
## 캔들 수집 범위·커버리지 (WO-FCE-OBSERVATION-INTEGRITY-01 Phase 2-2)

**가설 반증.** "1분봉 `count=200`(3.3시간)이라 미국 정규장 6.5시간을 못 채운다"는 사실이 아니었다.
수집기는 15초마다 200개를 다시 받으므로 **합집합이 정규장 전체를 덮는다.**

TSLA 1분봉 정규장(390분) 커버리지 실측:

| 날짜 | TSLA | LRCX | 비고 |
| --- | --- | --- | --- |
| 08-03 | 23.6% | 17.4% | KST 롤오버 결함 |
| 08-04 | 23.1% | 23.1% | 동일 |
| **08-05** | **100%** | **100%** | 롤오버 수리(`103fd09`) 당일 |

차트가 조각나 보인 원인은 캔들 조회 한도가 아니라 **수집 자체가 15:00 UTC 에 끊긴 것**이었다.
따라서 `count` 를 올리지 않는다 — 불필요한 TPS 부담이고 원인도 아니다.

그래도 커버리지 검사는 남긴다. 같은 종류의 정지가 재발하면 **차트가 조용히 이어 붙는 대신
갭으로 드러나야** 하기 때문이다(C3).

- `stock_paper_entry_chart` 응답에 `coverage {pct, observed, expected, gaps[], gap_count, missing_bars}`.
- 공백은 `backfillable`(200봉 창 이내) 여부를 함께 낸다. 창을 넘는 공백은 재수집으로 못 메우므로
  **갭으로 남긴다** — 없는 데이터를 지어내지 않는다.
- 차트 헤더에 `커버리지 N% · 결손 M봉(K구간)` 표기.
