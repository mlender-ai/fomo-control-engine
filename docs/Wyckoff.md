...
라이브 포지션 프로 화면의 패턴 매트릭스는 `1D / 12H / 4H / 1H / 15M`을 Bitget에서 각각 조회해 같은 와이코프 엔진으로 독립 분석한다. 기존 `wyckoff_mtf`처럼 한 캔들 집합을 내부 집계한 값과 구분한다.

- 각 시간봉은 완전히 종료된 확정 캔들만 사용한다. 진행 중인 마지막 봉은 레인지·이벤트·마커 계산에서 제외한다.
- 다른 시간봉에서 감지된 이벤트를 현재 차트에 투영하지 않는다. 매트릭스의 시간봉을 눌러 해당 캔들 차트로 전환한 뒤 표시한다.
- 한 시간봉의 데이터 부족은 그 행만 `unavailable`로 남기고 다른 시간봉 결과를 숨기지 않는다.
- 매트릭스는 기존 임계값을 그대로 사용하며 미검출을 피하려고 confidence를 낮추지 않는다.
