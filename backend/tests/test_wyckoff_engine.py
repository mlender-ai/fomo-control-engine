...
def test_wyckoff_v2_detects_spring_with_component_confidence() -> None:
    ...
    assert set(spring["components"]) == {"depth_significance", "return_speed", "volume_confirmation", "level_strength"}
...
def test_wyckoff_v2_detects_utad_with_component_confidence() -> None:
    ...
    assert set(utad["components"]) == {"depth_significance", "return_speed", "volume_confirmation", "level_strength"}
...
def test_wyckoff_v2_skips_events_when_no_range_exists() -> None:
    candles = [
        _candle(index, open_price=100 + index * 1.2, high=102 + index * 1.2, low=99 + index * 1.2, close=101 + index * 1.2, volume=900.0)
...
def test_short_position_gets_distribution_thesis_hook() -> None:
    structure = {
        ...
        "wyckoff": {
            ...
            "mtf": {"htf_phase": "distribution_phase_c", "htf_trend": "bearish", "alignment": "aligned"},
        },
    }
    indicators = {"last_close": 108.0, "bollinger_upper": 114.0, "bollinger_lower": 98.0}
...
def _range_candles(count: int) -> list[MarketCandle]:
    ...
    for index in range(count):
        ...
        candles.append(_candle(index, open_price=close - 0.4, high=high, low=low, close=close, volume=900.0 + (index % 4) * 70))
    return candles


def _levels() -> dict:
    return {
        "support": [{"price": 100.0, "score": 82, "touches": 5, "kind": "support", "sources": ["swing"]}],
        "resistance": [{"price": 110.0, "score": 78, "touches": 4, "kind": "resistance", "sources": ["swing"]}],
    }


def _candle(index: int, *, open_price: float, high: float, low: float, close: float, volume: float) -> MarketCandle:
    return MarketCandle(
