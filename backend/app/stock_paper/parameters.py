...
DEFAULT_PARAMETERS_PATH = Path(__file__).with_name("params") / "stock-v3.json"
...
def load_stock_parameters(path: Path = DEFAULT_PARAMETERS_PATH) -> StockPaperParameters:
    ...
    if parameters.version in {"stock-v2", "stock-v3"} and parameters.signature_gate_mode != "record_only":
        raise ValueError("stock signature status must remain record-only")
    if parameters.version in {"stock-v2", "stock-v3"} and parameters.earnings_gate_mode != "not_evaluable":
        raise ValueError("earnings must remain explicitly not_evaluable until a source is connected")
    if parameters.version == "stock-v3" and parameters.stance_gate_mode != "stable_long":
