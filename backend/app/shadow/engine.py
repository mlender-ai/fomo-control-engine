...
from app.db.models import ShadowAttribution, ShadowExtractRequest, ShadowProfile, ShadowRule, Trade
...
def extract_shadow_profile(trades: list[Trade], request: ShadowExtractRequest) -> ShadowProfile:
    ...
    if len(completed) < request.min_trades or len(profitable) < request.min_profitable_trades:
        raise ShadowSampleError(
            f"샘플이 부족합니다. 최소 완료 거래 {request.min_trades}건, 수익 거래 {request.min_profitable_trades}건이 필요합니다."
        )
    ...
    attribution = ShadowAttribution(
        noise_trades_pnl=round(sum(trade.pnl_amount for trade in completed if not _matches_any_rule(trade, rules)), 2),
        fomo_trades_pnl=round(sum(trade.pnl_amount for trade in fomo_trades), 2),
        late_exit_pnl=round(sum(trade.pnl_amount for trade in losing if trade.exit_score is not None and trade.entry_score is not None and trade.exit_score < trade.entry_score - 20), 2),
        counterfactual_trades=[
            {"trade_id": str(trade.id), "symbol": trade.symbol, "pnl_amount": trade.pnl_amount, "reason": "low_entry_score_loss"}
    ...
    return ShadowProfile(
        ...
        rules=rules,
        fomo_patterns=[
            {"pattern": "low_entry_score_loss", "count": len(fomo_trades), "pnl": attribution.fomo_trades_pnl}
        ],
        common_mistakes=[
            {"mistake": "entry_score_below_65", "count": len(fomo_trades)},
            {"mistake": "late_exit_after_score_drop", "count": len([trade for trade in losing if trade.exit_score and trade.entry_score and trade.exit_score < trade.entry_score - 20])},
        ],
...
def _rule(rule_id: str, text: str, conditions: dict, sample: list[Trade], all_trades: list[Trade]) -> ShadowRule:
