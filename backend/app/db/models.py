...
class Position(BaseModel):
    ...
    # **거래소에서 살아 있는 것을 마지막으로 확인한 시각** (2026-09-15).
    #
    # `synced_at` 은 동기화가 이 행을 만질 때마다 갱신된다 — 부재 카운트를 올리는 순회에서도
    # 갱신되므로 "마지막으로 본 시각"이 아니다. `closed_at` 은 **부재를 확정한 시각**이지
    # 청산 시각이 아니다. 둘 다 "언제 닫혔나"에 답하지 못한다.
    #
    # 워커가 09-12~09-14 정지한 뒤 재기동하자 그 사이 청산된 포지션들이 한꺼번에 종료 처리됐고,
    # 종료 빚 회수가 3일 전 청산을 "방금 종료"로 발송했다. 사용자가 즉시 알아봤다 —
    # "이건 옛날 거잖아". 창을 재려면 **살아 있는 것을 마지막으로 본 시각**이 필요하다.
    #
    # 거래소 목록에 실제로 나타났을 때만 갱신한다.
