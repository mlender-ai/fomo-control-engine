...
type SortKey = "setup_proximity_pct" | "long_score" | "short_score" | "prz_distance_pct" | "nearest_level_distance_pct" | "change_24h";
          ...
          <div className="scoutSearchResults">
            {results.map((item) => (
              <button key={item.symbol} onClick={() => void addSymbol(item.symbol)} type="button">
                ...
                <span>{item.base_coin || "-"} · {item.quote_coin || "-"}</span>
                <Star size={13} />
              </button>
            ))}
          </div>
        ...
        scanRows.length ? (
          <div className="scoutTableWrap">
            <table className="scoutTable">
              ...
              <tbody>
                {sortedRows.map((row) => (
                  <ScanRow key={row.symbol} row={row} scanReference={scannedAt} onOpen={() => setActiveSymbol(row.symbol)} onRemove={() => void removeSymbol(row.symbol)} />
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div className="scoutEmpty">
            <p>관심종목 {watchlist.length}개가 있습니다. 스캔을 눌러 6개 축으로 비교하세요.</p>
...
function ScanRow({ row, scanReference, onOpen, onRemove }: { row: ScoutScanRow; scanReference: string; onOpen: () => void; onRemove: () => void }) {
  ...
  if (row.error) {
    return (
      <tr className="scoutRow error">
        ...
        <td colSpan={9}>스캔 실패: {row.error}</td>
        <td>
  ...
  return (
    <tr className={`scoutRow ${stale ? "stale" : ""}`} onClick={onOpen}>
      <td><strong>{row.symbol}</strong></td>
