...
function PositionWhaleBanner({
  ...
  return (
    <section className={`positionWhaleBanner ${dominant}`} data-testid="position-whale-banner">
      <header>
        <div><Waves size={17} /><span>상위 고래 추적군 · 현재 미결제</span><strong>{summary.symbol}</strong></div>
        <small><i />30초 자동 갱신 · {summary.as_of ? shortWhaleTime(summary.as_of) : "공개 포지션 없음"}</small>
      </header>
      <div className="positionWhaleSummary">
        <div><span>추적군 롱</span><strong className="long">{compactWhaleMoney(summary.long_usd)}</strong><small>{summary.long_wallet_count}지갑</small></div>
        <div><span>추적군 숏</span><strong className="short">{compactWhaleMoney(summary.short_usd)}</strong><small>{summary.short_wallet_count}지갑</small></div>
      ...
      <footer>상위 추적군 20지갑 기준이며 전체 시장 분포가 아님 · 배너 = 현재 미결제 · 차트 별 = 과거 확정 체결 · 검증 전 자동 진입 근거로 사용하지 않음</footer>
    </section>
  );
}
...
function whaleMicroLabel(summary: PositionWhaleSummary | null): string {
  ...
  return `고래 추적군 L${summary.long_wallet_count}/S${summary.short_wallet_count}`;
}
