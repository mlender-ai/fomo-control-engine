        ...
        <div className="onchainWalletList">
          {filteredWallets.map((wallet) => (
            <section className={`onchainWalletRow ${wallet.review.state} trust-${wallet.review.trust_status}`} key={wallet.address}>
              <header><div><strong>{wallet.label}</strong><code>{wallet.address_short}</code><small>{wallet.leaderboard ? `리더보드 #${wallet.leaderboard.leaderboard_rank} · 계정 ${compactMoney(wallet.leaderboard.account_value_usd)} · ${selectionReason(wallet.leaderboard.selection_reason)}` : wallet.alias_disclaimer}</small></div><div><span>{whaleTrustState(wallet.review.trust_status)}</span><span className={`whaleParticipantTag ${wallet.participant_estimate?.follow_eligible ? "eligible" : "excluded"}`} title={wallet.participant_estimate?.reason ?? "유형 추정 근거 없음"}>{whaleParticipantLabel(wallet.participant_estimate?.participant_type ?? "unclassified")}{wallet.participant_estimate?.confidence ? ` ${wallet.participant_estimate.confidence}` : ""}</span><b>{wallet.review.win_1r_pct === null ? `추종 승률 대기 · N=${wallet.review.sample_size}` : `추종 승률 ${wallet.review.win_1r_pct}% · N=${wallet.review.sample_size}`}</b>{wallet.source === "discovery" ? <i className="onchainAutoTag">AUTO</i> : <button aria-label={`${wallet.label} 삭제`} disabled={busy} onClick={() => void removeWallet(wallet.address)} title="워치리스트에서 삭제" type="button"><Trash2 size={15} /></button>}</div></header>
